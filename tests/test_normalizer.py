...
def test_mcp_list_tools_become_gemini_functions_with_return_metadata():
    ...
    assert normalized["tool_output_types"]["mcp__docs__search"] == "mcp_call"
    assert normalized["tool_output_metadata"]["mcp__docs__search"]["server_label"] == "docs"


def test_mcp_call_history_reconstructs_qualified_function_call():
    normalized = normalize_responses_request(
        {
            "model": "gemini-3.8-flash",
            "input": [
                ...
                {"type": "function_call_output", "call_id": "mcp_call_1", "output": "found"},
            ],
        }
    )
    assert normalized["messages"][0]["function_call"]["name"] == "mcp__docs__search"
    assert normalized["messages"][0]["function_call"]["id"] == "mcp_call_1"
    assert normalized["messages"][1]["name"] == "mcp__docs__search"
