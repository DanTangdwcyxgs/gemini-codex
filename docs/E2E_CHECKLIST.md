...
4. Run a real Codex task with the Gemini profile. The acceptance task should force tool use:
...
Codex
  -> /v1/responses
  -> Gemini 3.8 Flash
  -> function_call / shell or file tool
...
6. Repeat with another small coding task to make sure the second tool round is not a one-off success.

7. Exercise `/v1/responses/compact` from Codex or a direct request. Treat the current `encrypted_content` field as compatibility transport, not as proof of OpenAI-compatible encryption semantics.
